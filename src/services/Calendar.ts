...
class Calendar {
  public static async sync(
    ...
    await Promise.all(
      _.map(items, async (entry: CalendarEntry) => {
        try {
          await dbPool.query(
          ...
          console.error(
            `Sync error: failed syncing event id=${entry.id}. Reason: ${error.message} `
          );
        }
      })
    );
  ...
  public static async addEntry(
    ...
    try {
      ...
      console.error(
        `Adding event failed: Failed to add calendar entry for user ${userId} with id ${entry.id}. Reason: ${error.message}`
      );
    }
  }
  ...
  public static async fetchEntries(userId: string): Promise<CalendarEntry[]> {
    try {
      const results = await dbPool.query(
        "SELECT summary, start_date, end_date, id FROM events WHERE user_id = $1",
        [userId]
      );
